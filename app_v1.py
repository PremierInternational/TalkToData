...
st.markdown("""
    ...
    .stApp {
        background-color: #ffebcd; /* Deep blue background */
    }
    .stTitle {
        color: #008000; /* White for titles */
        font-weight: bold;
        margin-bottom: 20px;
    }
    .stSubheader {
        color: #008000; /* White for subheaders */
        margin-top: 15px;
        margin-bottom: 10px;
    }
    .stTextInput > div > div > input {
        ...
        color: #1a237e; /* Dark blue text in input for contrast */
    }
    .stButton > button {
    ...
    .stMetric {
        background-color: #f0f8ff; /* Light blue for metrics */
        border-radius: 8px;
        padding: 15px;
        box-shadow: 0 3px 6px rgba(0,0,0,0.1);
        border: 1px solid #4caf50; /* Green border for metrics */
        color: #1a237e; /* Dark blue text in metrics for contrast */
    }
    .stDataFrame {
        border: 2px solid #4caf50; /* Green border for data tables */
        border-radius: 8px;
        overflow: hidden;
        background-color: #e8f5e9; /* Light green for data sections */
        padding: 10px;
        margin-top: 10px;
        color: #1a237e; /* Dark blue text in dataframes for contrast */
    }
    .sidebar .sidebar-content {
        background-color: #bbdefb; /* Slightly darker blue for sidebar */
        color: #1a237e; /* Dark blue text in sidebar */
    }
    .sidebar .stSelectbox, .sidebar .stCheckbox {
        background-color: #e8f5e9; /* Light green for sidebar elements */
        padding: 12px;
        border-radius: 8px;
        margin-bottom: 15px;
        border: 1px solid #4caf50; /* Green border */
        color: #1a237e; /* Dark blue text in selectbox/checkbox */
    }
    .stSpinner > div > div {
        color: #4caf50; /* Green spinner */
    }
    .stMarkdown h3, .stMarkdown h4 {
        color: #ffffff; /* White for markdown headers */
    }
    .stCheckbox > label {
...
def extract_condition(question: str, found_col: str) -> Tuple[Optional[str], Optional[Union[str, float, int]]]:
    ...
    m = re.search(r'(?:contains|has|include|includes)\s+["\']?(?P<val>.+)', q)
    if m:
    ...
    m = re.search(r'(?:more than|greater than|above|>)\s*(?P<num>[-+]?\d+(\.\d+)?)', q)
    if m:
